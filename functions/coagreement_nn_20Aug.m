 function [c,v,p,r,w,a] = coagreement_nn_20Aug(x,y,type,it)
...
%%% centering rule: mean/mean
           ...
           z1 = sum(cal.*weight)/sum(abs(cal.*weight));  %final output case o<>0
           end
           w=weight;
           a=[mean(x) mean(y)];
...
%%% centering rule: median/median
           ...
           z2 = sum(cal.*weight)/sum(abs(cal.*weight));  %final output case o<>0
           end
           w=[w weight];
...
%%% centering rule: mean/median
           ...
           z3 = sum(cal.*weight)/sum(abs(cal.*weight));  %final output case o<>0
           end
           w=[w weight];
           a=[a; mean(x) median(y)];
...
%%% centering rule: median/mean
           ...
           z4 = sum(cal.*weight)/sum(abs(cal.*weight));  %final output case o<>0
           end
           w=[w weight];
           a=[a; median(x) mean(y)];
