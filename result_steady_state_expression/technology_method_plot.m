function technology_method_plot(result)

result = 'result.mat';
load (result)

%data = {'CCLE_microarray', 'data_Atlas_RNAseq', 'FANTOM5_CAGE'};
...
for i = 1:length(data)
    ...
    for l = 2:size(plot_data,2)
        stat_v = [plot_data{2, l} plot_data{4, l} plot_data{6, l} plot_data{8, l}];
        error_v = [plot_data{3, l} plot_data{5, l} plot_data{7, l} plot_data{9,l}];
        y = [y; stat_v];
        e = [e; error_v];
    end
    max_v = [];
...
t = suptitle('Performance of correlations across different technologies');
set(t,'FontSize',30);
