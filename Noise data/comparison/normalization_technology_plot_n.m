function normalization_technology_plot_n(result)
...
norm_title = {'Unnormalized', 'LOG', 'ZSCORE'};
...
for i = 1:length(norm)
    ...
    idx_data = 2;
    for j = 1:length(data)
        exp_data = regexp(statistic_result(:,1) ,['(',data{j}, '_', norm{i},')'], 'match');
    ...
    title(norm_title{i},'Fontsize', 18)
    if i == 3
        legend(data_title,'Location', 'southoutside','Orientation','horizontal');
    end
    for t=1:7
...
t = suptitle('Performance of datasets across different normalizations');
set(t,'FontSize',30);
