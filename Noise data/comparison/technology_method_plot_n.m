function technology_method_plot_n(result)

result = 'result.mat';
load (result)

data = {'THP1_single_cell', 'insilico_size100_1', 'insilico_size100_2'};
data_title = {'THP1', 'Simulated data 1', 'Simulated data 2'};
norm = {'without', 'LOG', 'ZSCORE'};
norm_label = {'Unnormalzied', 'LOG', 'ZSCORE'};
corr = {'Pearson', 'Spearman', 'Distance','TSNI', 'GRNVBEM', 'SINCERITIES'};
...
for i = 1:length(data)
    ...
    idx_data = 2;
    for j = 1:length(corr)
        exp_data = regexp(statistic_result(:,1) ,['(?<=',data{i}, '_', ').*(?=',corr{j},')'], 'match');
        ...
        plot_data{idx_data, 1} = [corr{j}, '_std_error'];
    ...
    for l = 2:size(plot_data,2)
        stat_v = [plot_data{2, l} plot_data{4, l} plot_data{6, l} plot_data{8, l} plot_data{10, l} plot_data{12, l}];
        error_v = [plot_data{3, l} plot_data{5, l} plot_data{7, l} plot_data{9, l} plot_data{11, l} plot_data{13, l}];
        y = [y; stat_v];
        e = [e; error_v];
    end
    ...
    x = 1:7;
    h = bar(x,y);
    %set(h,{'Color'},{[0 0.4470 0.7410];[0.8500 0.3250 0.0980];[0.9290 0.6940 0.1250];[0.4940 0.1840 0.5560];[0.4660 0.6740 0.1880];[0.6350 0.0780 0.1840]},{'LineStyle'},{':';'--';'-.';'-';'-';'-'})
    ...
    h(5).FaceColor = [0.4660 0.6740 0.1880];
    h(6).FaceColor = [0.6350 0.0780 0.1840];
    set(gca, 'XTickLabel', statistics_properties, 'Fontsize', x_fontsize);
    title(data_title{i},'Fontsize', 18)
    if i == 3
        legend(corr,'Location', 'southoutside','Orientation','horizontal');
    end
    for t=1:7
...
t = suptitle('Performance of methods across different datasets');
set(t,'FontSize',30);
