function method_technology_plot_n(result)
...
corr = {'Pearson', 'Spearman', 'Distance'};
...
for i = 1:length(corr)
    ...
    idx_data = 2;
    for j = 1:length(data)
        exp_data = regexp(statistic_result(:,1) ,['(?<=',data{j}, '_', ').*(?=',corr{i},')'], 'match');
    ...
    title(corr{i},'Fontsize', 18)
    if i == 3
        legend(data_label,'Location', 'southoutside','Orientation','horizontal','Fontsize',15);
    end
...
t = suptitle('Performance of datasets across different correlations');
set(t,'FontSize',30);
