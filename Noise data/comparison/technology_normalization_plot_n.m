function technology_normalization_plot_n(result)
...
norm_label = {'Unnormalized', 'LOG', 'ZSCORE'};
...
for i = 1:length(data)
    plot_data = {};
    plot_data(1,:) = {'type', 'Sensitivity', 'Specificity', 'F1-Score for upregulation', 'F1-Score for downregulation', 'AUC', 'Precision-positive', 'Precision-negative'};

    idx_data = 2;

    for j = 1:length(norm)
        exp_data = regexp(statistic_result(:,1) ,['(',data{i}, '_', norm{j},')'], 'match');
        ...
        plot_data{idx_data, 1} = [norm{j}, '_std_error'];
    ...
    if i == 3
        legend(norm_label,'Location', 'southoutside','Orientation','horizontal');
    end
...
t = suptitle('Performance of normalizations across different datasets');
set(t,'FontSize',30);
